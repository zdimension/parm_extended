...
def build_instruction_table():
    ...
    for k, v in ParsingData.INSTR_DEFS.items():
        k = k.replace("[", "\\[\\s*").replace("]", "\\s*\\]")
        for rg, sub in ParsingData.REGEXES.items():
            k = rg.sub(sub, k)
...
ParsingData.INSTRUCTIONS = build_instruction_table()
    ...
    for k, v in ParsingData.INSTR_DEFS.items():
        ...
        for arg in reversed(v):
            if type(arg) == int:
                ...
                label = read_op(int(ParsingData.RE_INT.search(arg).group(1)))
                ops[arg] = f"0x{label:04x}"
            elif arg.startswith("imm"):
                val = read_op(int(ParsingData.RE_INT.search(arg).group(1)))
                if arg[3] in ("w", "p"):
...
class Lookup:
    def __init__(self, state: "AssemblerState"):
        self.state = state
    ...
    def __getitem__(self, key: Sanitized):
        ...
        sym = None
        try:
            sym = self.state.get_symbol(key)
        except KeyError:
            if self.state.wildcard_regex.match(key):
                for k, v in self.state.WILDCARDS.items():
                    if (k[-3:] == "XXX" and key.startswith(k[:-3])) or (k[:3] == "XXX" and key.endswith(k[3:])):
                        sym = v
                        break
        if sym is None:
            raise KeyError(f"Symbol {key} not found")
        sym.refs.add(self.state.lineno)
        self.state.line_refs[self.state.lineno].add(sym)
        if self.state.current_function is not None:
            sym.refsfn.add(self.state.current_function)
...
*includes, main_file = cli_args.files
...
class AssemblerState:
    ...
    def __init__(self, lines):
        self.optim_done = False
        self.out = []
        self.lines = lines
        self.lines += [".$END:"]
        self.lines_tok = tokenize_all(lines)
        self.lookup = Lookup(self)
        self.line_refs = defaultdict(set)
    ...
    def get_full_current_scope(self):
        return {**self.GLOBAL_SCOPE, **self.file_scopes[self.current_file]}

    def get_all_file_scopes(self):
        return {**self.file_scopes}

    def add_label(self, name, value):
        sym = self.add_symbol(name)
    ...
    def get_symbol(self, name):
        if name != name.strip():
            breakpoint()
        name = sanitize(name)
        if sym := self.GLOBAL_SCOPE.get(name):
            return sym
        if sym := self.file_scopes[self.current_file].get(name):
            return sym

        raise KeyError(f"Symbol {unsanitize(name)} not found from {self.current_file}")

    def add_symbol(self, oname, set_global=False):
        wildcard = False
        if "XXX" in oname:
            wildcard = True
            set_global = True  # XXX symbols are always global since they're hacks
        name = sanitize(oname)
        if glob_sym := self.GLOBAL_SCOPE.get(name):
            if glob_sym.source == self.current_file:
                return glob_sym
            elif glob_sym.is_weak:
                new_sym = Symbol(oname, source=self.current_file, line_start=self.lineno, line_end=self.lineno)
                self.GLOBAL_SCOPE[name] = new_sym
                self.file_scopes[self.current_file][name] = new_sym
                if wildcard:
                    self.add_wildcard(name, new_sym)
                return new_sym
            else:
                raise Exception(f"Symbol {name} already globally defined in {glob_sym.source}")

        sym = self.file_scopes[self.current_file].get(name)
        if not sym:
            sym = Symbol(oname, source=self.current_file, line_start=self.lineno, line_end=self.lineno)
            self.file_scopes[self.current_file][name] = sym
        if wildcard:
            self.add_wildcard(name, sym)
        if set_global:
            self.GLOBAL_SCOPE[name] = sym
        return sym

    def add_wildcard(self, name, new_sym):
        self.WILDCARDS[name] = new_sym
        self.wildcard_regex = re.compile(
            r"^(" + "|".join(re.escape(name).replace("XXX", ".*?") for name in self.WILDCARDS) + r")$")

    def current_function_name(self):
        if self.current_function is not None:
            return self.current_function.name
        return "<none>"

    def parse_imm(self, s):
        if not s:
            return 0
        ss = sanitize(s)
        # try:
        ...
        try:
            if ParsingData.RE_IDENT.match(s):
                res = self.lookup[ss]
            else:
                res = eval(ss, {}, self.lookup)
        except Exception as e:
            raise AsmException(f"In {self.current_file}: {e} while parsing immediate value {s}") from e
        if type(res) == float and res != (res := int(res)):
            raise AsmException(s)
        return res

    def insert_line(self, i, val):
        self.lines.insert(i, val)
        self.lines_tok.insert(i, tokenize(val))

    def set_line(self, i, val):
        self.lines[i] = val
        self.lines_tok[i] = tokenize(val)

    def do_func_optim(self):
        print("Optimizing functions...")
        useful = defaultdict(set)
        # useful = [False] * len(lines)
        for file, symbols in self.get_all_file_scopes().items():
                ...
                for name, sym in list(symbols.items()):
                    if len(sym.refs) == 0:
                        ...
                        if not outside_refs:
                            ...
                            for line in sym.line_range():
                                if self.lines[line] is None:
                                    continue
                                self.lines[line] = None
                                self.lines_tok[line] = None
                                for refd in self.line_refs[line]:
                                    refd.refs.discard(line)
                        else:
                            print(
                                f"Unused function {unsanitize(name)} ({sym.line_range()}) from {sym.source} contains referenced symbols: {','.join(s.name for s in outside_refs[:10])}")
            if not removed_syms:
                break
            for file, symbols in self.get_all_file_scopes().items():
    ...
    def try_assemble(self, pc: int, m: re.Match, instr: str, output, line: str, line_num: int) -> Sequence[tuple[
        int, int, str, str]] | None:
        dic = m.groupdict()
        for k, v in dic.items():
            if k.startswith("imm"):
                if v is not None and any(re.match(r.replace("\\1", "test"), v) for _, r in ParsingData.REGEXES_REGS):
                    return None
                kw = k[3:]
                off = 0
                ...
                if k[3] == "p":
                    off = -8
                width = int(kw)
                dic[k] = (self.parse_imm(v) + off, width)
                valmax = 2 ** (width + fac)
                ...
                if k[0] == "R":
                    ...
                    try:
                        dic[k] = (ParsingData.CONDS.index(ParsingData.CONDS_ALIAS.get(v, v)), 4)
                    ...
                    try:
                        ...
                        lbl_value = self.lookup[sanitize(v)] // 2
                        if pcrel:
                            val = lbl_value - ((pc + 2) & ~1)
                            if val < 0:
                                raise AsmException(
                                    f"PC-relative label {v} is before current PC {pc} (Thumb-16 encoding only supports 0..1020)")
                        ...
                        if val not in valrange:
                            if cli_args.optimize_functions and not self.optim_done:
                                continue
                            if cli_args.ignore_large_jumps:
                                print("ignoring oversize", line, " in ", self.current_function_name())
                                continue  # we'll see later
                            if not cli_args.notrampo:
                                new_label = "trampo_" + ''.join(
                                    random.choices(string.ascii_lowercase + string.digits, k=5))
                                # thanks @Guekka
                                print("Trampolining:", line, "while in", self.current_function_name())
                                print("Old code:", self.lines[line_num - 1:line_num + 8])
                                if cond and len(instr) == 3:
                                    ...
                                    cond_id = dic["cond"][0]
                                    opp_id = cond_id ^ 1
                                    self.set_line(line_num - 1, f"b{ParsingData.CONDS[opp_id]} {new_label}")
                                    self.insert_line(line_num, f"{new_label}:")
                                    self.insert_line(line_num, f"b {v}")
                                    print("New code:", self.lines[line_num - 1:line_num + 2])
                                    raise Trampoline(2)
                                elif instr == "ldr":
                                    """
                                    ldr Rd, label_value
                                    ...
                                    rd = f"r{dic['Rd'][0]}"
                                    self.set_line(line_num - 1, f"ldr {rd}, {tr_data}")
                                    self.insert_line(line_num, f"{tr_after}: ldr {rd}, [{rd}]")
                                    self.insert_line(line_num, f"{tr_data}: .long {v}")
                                    self.insert_line(line_num, f".p2align 2")
                                    self.insert_line(line_num, f"b {tr_after}")
                                    print("New code:", self.lines[line_num - 1:line_num + 4])
                                    raise Trampoline(4)
                                elif instr == "adr":
                                    """
                                    adr Rd, label_value
                                    ...
                                    rd = f"r{dic['Rd'][0]}"
                                    self.set_line(line_num - 1, f"ldr {rd}, {tr_data}")
                                    self.insert_line(line_num, f"{tr_after}: ")
                                    self.insert_line(line_num, f"{tr_data}: .long {v}")
                                    self.insert_line(line_num, f".p2align 2")
                                    self.insert_line(line_num, f"b {tr_after}")
                                    print("New code:", self.lines[line_num - 1:line_num + 4])
                                    ...
                                    tr_after = f"{new_label}_after"
                                    self.set_line(line_num - 1, f"mov r12, r7")
                                    self.insert_line(line_num, f"add pc, r11")
                                    self.insert_line(line_num, f"mov r7, r12")
                                    self.insert_line(line_num, f"{tr_after}: mov r11, r7")
                                    self.insert_line(line_num,
                                                     f"{tr_target}: .long {v} - CURPC - 2*(6)")  # because the first .long is 6 instructions before the end
                                    self.insert_line(line_num, f".p2align 2")
                                    self.insert_line(line_num, f"b {tr_after}")
                                    self.insert_line(line_num, f"ldr r7, {tr_target}")
                                    print("New code:", self.lines[line_num - 1:line_num + 7])
                                    raise Trampoline(7)
                            raise Exception(
                                f"Jump too wide : {lbl_value} is {dic[k][0]} which does not fit in {width} bits")
                        if not cli_args.nojumps:
                            self.jumps.append((pc, lbl_value))
                    except KeyError:
    ...
    def assemble(self, line: str, pc: int, line_num: int, tokens: list["Token"]) -> Sequence[tuple[int, int, str, str]]:
        if cli_args.optimize_functions and not self.optim_done:
            instr, *args = tokens
            for arg in args:
                if arg[0] == "#":
                ...
                if len(arg) == 1:
                    ...
                    if ParsingData.RE_REGISTER.match(arg):
                        continue
                try:
                    self.parse_imm(arg)
                except AsmException:
        ...
        if instr[0] == "$":
            ...
            if instr == "$short":
                ...
                return (pc, n, dl, n),
            if instr == "$bytes":
                first, second = map(self.parse_imm, args_split)
            ...
            if instr.startswith("$bl"):
                ...
                if first:
                    ...
                    if not cli_args.nojumps:
                        self.jumps.append((pc, self.parse_imm(args) // 2))
                return (pc, val, dl, f"{n} ({2 * n:x})"),
            if instr.startswith("$bcond"):
                first = instr[6] == "1"
                cond, targ = args.strip().split(None, 1)
                n = self.parse_imm(targ) // 2 - (pc + (1 if first else 0)) - 1
                if first:
                    val = (0b1111_0 << 11) | ((n >> 11) & 0b111_1111_1111)
                else:
                    val = (0b1111_1 << 11) | (n & 0b111_1111_1111)
                    if not cli_args.nojumps:
                        self.jumps.append((pc, self.parse_imm(targ) // 2))
                return (pc, val, dl, f"{n} ({2 * n:x})"),
        ...
        exc = None
        while True:
            for i, output in ParsingData.INSTRUCTIONS[line[0]].items():
                if m := i.match(line):
                    if type(output) is str:
                        ...
                        try:
                            if (res := self.try_assemble(pc, m, instr, output, line, line_num)) is not None:
                                return res
                        except Exception as e:
                            exc = e
            else:
        ...
        while True:
            ...
            try:
                ...
                for lineno_, (line, tokens) in enumerate(zip(self.lines, self.lines_tok)):
                    ...
                    match tokens:
                        case ["#file", name]:
                            self.current_file = eval(name.strip())
                            self.file_scopes[self.current_file] = {}
                            add_instr(line, size=0, tokenized=tokens)
                            continue
    
                    while tokens:
                        match tokens:
                            case [label, ":", *rest] if ParsingData.RE_LBL_NAME.match(label):
                                lbl_name = sanitize(label)
                                self.add_label(lbl_name, current_pc())
                                tokens = rest

                                def fixup(name, meth):
                                    for sym in self.get_full_current_scope().keys():
                                        if meth(sym, name):
                                            self.add_label(sym, current_pc())
                    ...
                    def process_tokens():
                        ...
                        match tokens:
                            case ["dmb", *rest]:
                                ...
                                add_instr("nop")
                            case ["bl" | "blx", arg] if not ParsingData.RE_REGISTER.match(arg):
                                add_instr(f"$bl1 {arg}")
                                add_instr(f"$bl2 {arg}")
                            ...
                            case [".hidden", sym_name]:
                                self.add_symbol(sym_name).is_hidden = True
                            case [".globl", sym_name]:
                                self.add_symbol(sym_name, True)
                            case [".weak", sym_name]:
                                self.add_symbol(sym_name, True).is_weak = True  # weak are global by default apparently
                            ...
                            case [".fnstart"]:
                                ...
                                sym = self.get_symbol(current_function_preproc)
            ...
            for instr, regs in pushpopcode:
                ...
                sym = self.add_symbol(ppfname, True)
                sym.start = current_pc()
                ...
                try:
                    if val is not None:
                        ...
                        match cmd:
                            ...
                            case "#fnbegin":
                                self.current_function = self.get_symbol(arg)
                            case "#fnend":
                                self.current_function = None
                            case "#section":
                    ...
                    print(f"Build error on line {self.lineno}: {line} (inside {self.current_function_name()})")
                    raise
            ...
            if trampo_offset != 0:
                ...
                if cli_args.optimize_functions and not self.optim_done:
                    passes = self.do_func_optim()
                    self.lines = [l for l in self.lines if l]
                    self.lines_tok = [l for l in self.lines_tok if l]
                    if passes >= 1:
...
def write_log():
    disp_log = asm_state.instr_log.copy()
    jump_delta = 0
    if cli_args.function:
        try:
            fsym = asm_state.get_symbol(sanitize(cli_args.function))
        except KeyError as e:
            matches = [(nam, sym) for symbols in asm_state.get_all_file_scopes().values() for nam, sym in
                       symbols.items() if
                       cli_args.function in nam]
            match matches:
                case [(_, single)]:
                    fsym = single
                case []:
    ...
    root = len(columns) + 1
    for depth, (src, dst) in enumerate(asm_state.jumps):
        src = src - jump_delta
        dst = dst - jump_delta
...
write_out(asm_state.out)

# with open(os.path.join(os.path.dirname(__file__), "lisp.s.raw"), "rb") as tf:
#     test_bytes: bytes = tf.read()

out_bytes = bytes(byte for word in asm_state.out for byte in (word & 0xff, word >> 8))
...
def gen_call_graph():
    ...
    with open(os.path.splitext(main_file)[0] + ".dot", "w") as fo:
        fo.write("digraph {\n")
        for file, symbols in asm_state.get_all_file_scopes().items():
            for name, sym in symbols.items():
...
def digital_debugger(out):
    ...
    while True:
        ...
        if pcfix < len(funcs):
            func = funcs[pcfix]
            _, _, code, data = asm_state.instr_log[pcfix]
        else:
            func = None
...
if cli_args.debug:
    digital_debugger(asm_state.out)
