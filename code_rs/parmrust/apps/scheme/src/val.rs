...
use core::ptr;
use parm::heap::budmap::BudMap;
use parm::heap::string::{FromStr, String};
use parm::{makestr, print, tty};
use parm::tty::{DisplayTarget, ParmDisplay};
use crate::env::SchemeEnv;
use crate::eval::CallEvaluation;
...
macro_rules! lisplist {
    () => {
       $crate::val::LispList::Empty
    };
    ($item:expr) => {
        $crate::val::LispList::singleton($item.into())
    };
    ($item:expr, $($rest:expr),*) => {
        $crate::val::LispList::Cons($item.into(), $crate::val::LispVal::List(lisplist!($($rest),*)).into())
    };
}
