...
macro_rules! stack_buffer {
    {
        ...
        #[derive(Clone)]
        $($pub)? struct $name {
            buf: [$crate::vendor::midly::num::u7; $size],
            len: usize,
        }
        ...
        impl $name {
            ...
            $($pub)? const fn new() -> $name {
                $name {
                    buf: [$crate::vendor::midly::num::u7::new(0); $size],
                    len: 0,
                }
            }
        }
        ...
        impl $crate::vendor::midly::stream::Buffer for $name {
            #[inline]
            fn push(&mut self, data: &[$crate::vendor::midly::num::u7]) -> core::result::Result<(), ()> {
            ...
            #[inline]
            fn as_slice(&self) -> &[$crate::vendor::midly::num::u7] {
                &self.buf[..self.len]
            }
        }
    };
    {
        ...
        $crate::vendor::midly::stack_buffer!(@impl_def {$($attr)*} {} {$name} {$size});
    };
    {
        $(#[$attr:meta])*
        pub struct $name:ident([u8; $size:expr]);
    }=> {
        $crate::vendor::midly::stack_buffer!(@impl_def {$($attr)*} {pub} {$name} {$size});
    };
}
