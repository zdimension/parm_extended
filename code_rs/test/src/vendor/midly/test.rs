use crate::vendor::midly::{EventIter, Result as MidlyResult, TrackEvent};
...
impl crate::vendor::midly::io::Write for Vec<u8> {
    type Error = &'static str;
    type Seekable = crate::vendor::midly::io::NotSeekable<Self>;
...
mod parse_collect {
    ...
    impl<'a> Smf<'a> {
        pub fn parse(raw: &[u8]) -> MidlyResult<Smf> {
            let (header, tracks) = crate::vendor::midly::parse(raw)?;
            Ok(Smf {
                header,
                tracks: tracks
...
mod parse_lazy {
    ...
    pub struct Smf<'a> {
        pub header: crate::vendor::midly::Header,
        pub tracks: crate::vendor::midly::TrackIter<'a>,
    }
    impl Smf<'_> {
        pub fn parse(raw: &[u8]) -> MidlyResult<Smf> {
            let (header, tracks) = crate::vendor::midly::parse(raw)?;
...
fn test_live_api(file: &str) {
    ...
    #[cfg(feature = "alloc")]
    let arena = crate::vendor::midly::Arena::new();
    for (bytes, ev) in smf.tracks.iter().flat_map(|track| track.iter()) {
        use crate::vendor::midly::{
...
fn test_stream_api(file: &str) {
    ...
    while !byte_stream.is_empty() {
        ...
        use crate::vendor::midly::primitive::SplitChecked;
...
mod parse {
    ...
    fn not_midi() {
        ...
        match result {
            ...
            Err(err) => match err.kind() {
                crate::vendor::midly::ErrorKind::Invalid(_) => {}
                crate::vendor::midly::ErrorKind::Malformed(_) => {
    ...
    #[test]
    fn stack_buf() {
        use crate::vendor::midly::{num::u7, stack_buffer, stream::Buffer};
        stack_buffer! {
    ...
    fn arena_is_send() {
        ...
        assert_send::<crate::vendor::midly::Arena>();
    }
}
