...
use crate::parm::heap::HeapAllocator;
...
impl Vec {
    ...
    fn grow(&mut self) {
        ...
        self.ptr = match NonNull::new(new_ptr as *mut u32) {
            Some(p) => p,
...
fn main() {
    parm::heap::init();
    /*let buf = parm::heap::malloc(4);
    let buf = parm::heap::realloc(buf, 12);
    unsafe {
        *buf.add(0) = 0xab;
        *buf.add(1) = 0xcd;
        *buf.add(2) = 0xef;
    }
    let buf = parm::heap::realloc(buf, 24);
    unsafe {
        *buf.add(3) = 0x12;
        *buf.add(4) = 0x34;
        *buf.add(5) = 0x56;
