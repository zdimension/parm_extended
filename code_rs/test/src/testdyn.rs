...
fn main() {
    ...
    for track in UnwrapIter::new(tracks) {
        for ev in UnwrapIter::new(track) {
            sleep(ev.delta.as_int() as usize * 6);
            match ev.kind {
                TrackEventKind::Midi { channel, message } => {
                    print!(channel.as_int() as u32, ":");
                    match message {
                        ...
                        MidiMessage::Controller { .. } => {}
                        MidiMessage::ProgramChange { program } => {
                            set_instr(MidiInstrument::from_byte(program.as_int()));
                        }
                        MidiMessage::ChannelAftertouch { .. } => {}
                        MidiMessage::PitchBend { .. } => {}
                    }
                ...
                TrackEventKind::Meta(msg) => {
                    match msg {
                        ...
                        MetaMessage::MidiPort(_) => println!("midi port"),
                        MetaMessage::EndOfTrack => println!("end of track"),
                        MetaMessage::Tempo(_) => println!("tempo"),
                        MetaMessage::SmpteOffset(_) => println!("smpte offset"),
    ...
    /*set_instr(MidiInstrument::Piano1);
    let mut note = midi!(C, 4);
    for i in 0..40 {
        set_note(note);
        note += Interval::PERFECT_FIFTH;
        press_key();
