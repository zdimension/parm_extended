...
pub struct CParser<'a> {
    code: &'a [char],
    iter: Peekable<Enumerate<Copied<Iter<'a, char>>>>,
    scope: Scope
}
...
impl<'a> CParser<'a> {
    pub fn new(s: &'a [char]) -> Self {
        ...
        CParser {
            code: s,
            iter: s.iter().copied().enumerate().peekable(),
            scope
        }
    }
    ...
    fn read_boolean(&mut self) -> Result<LispVal, ReadError> {
        if self.accept('t') {
            ...
            Err(ReadError::BoolParseError(self.iter.peek().map(|&(_, ch)| ch)))
        }
    }
